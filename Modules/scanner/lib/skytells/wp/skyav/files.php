<?php
/**
 * Skytells-AV Security Scanner Filesystem.
 *
...
class files {
	...
	const TMP_FILES = 'skyav_tmp_files';
	...
	public static function init() {
		if (!static::$_init) {
			...
			define('SKYELLSAV_REGEX_ABSPATH', '/^([a-z]:)?' . preg_quote(static::$abspath, '/') . '/ui');

			// Match restricted paths.
			define('SKYELLSAV_REGEX_RESTRICTED', '/^(' . preg_quote(static::get_admin_dir(), '/') . '|' . preg_quote(static::get_includes_dir(), '/') . ')/ui');
			define('SKYELLSAV_REGEX_RESTRICTED_UPLOADS', '/^' . preg_quote(static::get_uploads_dir(), '/') . '.*\.php$/uiS');
	...
	public static function get_tmp_dir() {
		if (is_null(static::$tmp_dir)) {
			static::$tmp_dir = static::get_base_dir() . static::get_uploads_dir() . '_sky-av/';
	...
	public static function sanitize_relative(&$path) {
		...
		$path = preg_replace(SKYELLSAV_REGEX_ABSPATH, '', $path);
		...
		try {
			...
			while (false !== ($entry = readdir($handle))) {
				...
				if (@is_file($file)) {
					...
					$file = preg_replace(SKYELLSAV_REGEX_ABSPATH, '', $file);
			...
			if (
				...
				try {
					if (false !== ($md5 = @md5_file($v))) {
						...
						$v = preg_replace(SKYELLSAV_REGEX_ABSPATH, '', $v);
						$out[$v] = $md5;
					}
				} catch (\Throwable $e) {
					continue;
	...
	public static function info($path, $conditional=false) {
		...
		$out['path'] = preg_replace(SKYELLSAV_REGEX_ABSPATH, '', $path);
	...
	public static function is_ignored($path) {
		...
		if (strpos($path, 'skytells-guard') !== false) { return true; }
		if (!static::can_write()) {
			return true;
		}
	...
	public static function is_core_path($path) {
		...
		// Some easy checks.
		if (
			preg_match(SKYELLSAV_REGEX_RESTRICTED, $path) ||
			(('index.php' !== basename($path)) && preg_match(SKYELLSAV_REGEX_RESTRICTED_UPLOADS, $path))
		) {
			return true;
		}
