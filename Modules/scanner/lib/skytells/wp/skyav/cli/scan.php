...
namespace skytells\wp\skyav\cli;

use \skytells\wp\skyav\files;
use \skytells\wp\skyav\options;
use \skytells\wp\skyav\scan as file_scan;
...
class scan extends \WP_CLI_Command {
	...
	public function custom($args=null, $assoc_args=array()) {
		...
		if (false === ($scan = file_scan::get_open_scan())) {
			...
			if (!$scan->save(array('scan'=>$session))) {
				WP_CLI::error(
					__('A scan could not be started.', SKYELLSAV_L10N)
				);
			}
			if ($options['summary']) {
				WP_CLI::success(
					sprintf(
						__('Starting Scan #%d.', SKYELLSAV_L10N),
						$scan->get_scan_id()
					)
				);
			}
		...
		elseif ($options['summary']) {
			WP_CLI::success(
				sprintf(
					__('Resuming Scan #%d.', SKYELLSAV_L10N),
					$scan->get_scan_id()
				)
			);
		}
		...
		if ($options['summary']) {
			...
			if ($scan->get_option('skip_cache')) {
				$skipping[] = __('cache', SKYELLSAV_L10N);
			}
			if ($scan->get_option('skip_media')) {
				$skipping[] = __('audio/video', SKYELLSAV_L10N);
			}
			if ($scan->get_option('skip_images')) {
				$skipping[] = __('images', SKYELLSAV_L10N);
			}
			if (count($scan->get_option('blacklist'))) {
				$skipping[] = __('other', SKYELLSAV_L10N);
			}
			if (count($skipping)) {
				WP_CLI::log(
					__('Skipping:', SKYELLSAV_L10N) . ' ' . implode(', ', $skipping)
				);
			}
			...
			if ($size > 0) {
				...
				WP_CLI::log(
					sprintf(
						__('Max File Size: %.1f MB', SKYELLSAV_L10N),
						$size
					)
				);
			}
		...
		WP_CLI::success(
			sprintf(
				__('Finished Scan #%d.', SKYELLSAV_L10N),
				$scan->get_scan_id()
			)
		);
	...
	public function _list() {
		...
		if (!count($dbResult)) {
			WP_CLI::error(
				__('No scans have been completed.', SKYELLSAV_L10N)
			);
		}

		$headers = array(
			__('Scan ID', SKYELLSAV_L10N),
			__('Finished', SKYELLSAV_L10N),
			__('Elapsed', SKYELLSAV_L10N),
			__('Status', SKYELLSAV_L10N),
			__('Files', SKYELLSAV_L10N),
			__('Scanned', SKYELLSAV_L10N),
			__('Skipped', SKYELLSAV_L10N),
			__('Warnings', SKYELLSAV_L10N),
		);
		$data = array();
		...
		WP_CLI::success(
			__('Found scans', SKYELLSAV_L10N) . ': ' . count($dbResult)
		);
	...
	public function view($args=null, $assoc_args=array()) {
		...
		if (!$scan->is_scan()) {
			WP_CLI::error(
				__('Invalid scan.', SKYELLSAV_L10N)
			);
		}
		elseif ('finished' !== $scan->get_status()) {
			WP_CLI::error(
				__('This scan has not yet completed.', SKYELLSAV_L10N)
			);
		}
		...
		if (!$options['files'] && !$options['summary'] && !$options['warnings']) {
			WP_CLI::error(
				__('You have disabled all output. Haha.', SKYELLSAV_L10N)
			);
		}
		...
		if ($options['warnings'] && $results['results']['warnings']) {
			$headers = array(
				__('Warning', SKYELLSAV_L10N),
				__('Core', SKYELLSAV_L10N),
				__('File', SKYELLSAV_L10N),
			);
			$data = array();
			foreach ($results['details']['warnings'] as $v) {
				$data[] = array(
					'Warning'=>$v['warningText'],
					'Core'=>($v['type'] ? __('Yes', SKYELLSAV_L10N) : ''),
					'File'=>$v['file'],
				);
			}
			WP_CLI::log('');
		...
		if (
			...
			foreach ($fields as $k=>$v) {
				foreach ($results['details'][$k] as $v2) {
					$data[] = array(
						'Status'=>$v,
						'Core'=>($v2['type'] ? __('Yes', SKYELLSAV_L10N) : ''),
						'File'=>$v2['file'],
					);
				}
			}
