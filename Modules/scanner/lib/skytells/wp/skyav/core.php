<?php
/**
 * Skytells-AV Security Scanner Core.
 *
...
class core {
	...
	public static function load_core_checksums($refresh=false) {
		...
		if (!is_array($tmp) || !count($tmp)) {
			return new WP_Error(
				'api',
				__('The WordPress.org API could not be reached.', SKYELLSAV_L10N)
			);
		}
		...
		foreach ($inserts as $v) {
			$wpdb->query("
				INSERT INTO `{$wpdb->prefix}skyav3_core` (`type`, `slug`, `version`, `file_hash`, `file_path`, `checksum`)
				VALUES " . implode(',', $v)
			);
		}
		...
		if (!static::$core['checksums']) {
			return new WP_Error(
				'database',
				__('The WordPress Core checksums could not be saved.', SKYELLSAV_L10N)
			);
		}
	...
	protected static function get_plugin_remote($slug='') {
		$transient_key = 'skyav_remote_plugin_' . static::md5($slug);
	...
	protected static function get_theme_remote($slug='') {
		$transient_key = 'skyav_remote_theme_' . static::md5($slug);
		if (false === ($out = get_transient($transient_key))) {
			@require_once(ABSPATH . 'wp-admin/includes/theme.php');
	...
	public static function analyze_ssl() {
		...
		$url = 'http:' . SKYELLSAV_PLUGIN_URL . 'img/skytells.svg';
		if (extension_loaded('curl')) {
			try {
				...
				curl_setopt($ch, CURLOPT_USERAGENT, 'Skytells-AV');
	...
	public static function analyze_index() {
		$url = (is_ssl() ? 'https:' : 'http:') . SKYELLSAV_PLUGIN_URL . 'img/';
		$response = wp_remote_get($url);
		if (200 === wp_remote_retrieve_response_code($response)) {
			$response = wp_remote_retrieve_body($response);
			return (false === common\mb::strpos($response, 'skytells.svg'));
		}
	...
	public static function get_vulnerabilities($slug, $type='plugin') {
		...
		$transient_key = "skyav_wpvulndb_{$type}_" . static::md5($slug);
		if (false === ($out = get_transient($transient_key))) {
	...
	public static function get_vulnerabilities_url($slug, $type='plugin') {
		...
		$transient_key = "skyav_wpvulndburl_{$type}_" . static::md5($slug);
