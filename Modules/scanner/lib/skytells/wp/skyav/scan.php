...
class scan {
	...
	public function __call($method, $args) {
		...
		throw new \Exception(
			sprintf(
				__('The required method "%s" does not exist for %s', SKYELLSAV_L10N),
				$method,
				get_called_class()
			)
		);
	...
	public function get_progress() {
		...
		if ($this->is_scan()) {
			...
			if ('scanning' === $this->get_status()) {
				$total = (int) $wpdb->get_var("SELECT COUNT(*) FROM `{$wpdb->prefix}skyav3_scan_files` WHERE `scan_id`={$progress['scan_id']}");
				$scanned = (int) $wpdb->get_var("SELECT COUNT(*) FROM `{$wpdb->prefix}skyav3_scan_files` WHERE `scan_id`={$progress['scan_id']} AND `status` != ''");
	...
	public function get_results() {
		...
		$warnings = array(
			'chmod'=>__('File Permissions', SKYELLSAV_L10N),
			'chown'=>__('File Owner', SKYELLSAV_L10N),
			'core-extra'=>__('Extra Core File', SKYELLSAV_L10N),
			'core-missing'=>__('Missing Core File', SKYELLSAV_L10N),
			'core-modified'=>__('Modified Core File', SKYELLSAV_L10N),
			'core-old'=>__('Deprecated Core File', SKYELLSAV_L10N),
			'dev-file'=>__('Developer/System File', SKYELLSAV_L10N),
			'mime'=>__('File Type', SKYELLSAV_L10N),
			'php-eval'=>__('eval()', SKYELLSAV_L10N),
			'php-gibberish'=>__('Unusual Data', SKYELLSAV_L10N),
			'php-phpinfo'=>__('phpinfo()', SKYELLSAV_L10N),
			'php-system'=>__('Unix System Call', SKYELLSAV_L10N),
			'timeout'=>__('Timeout Error', SKYELLSAV_L10N),
		);
		...
		if (!$out['error']) {
			...
			$dbResult = $wpdb->get_results("
				SELECT
					`status`,
					COUNT(*) AS `count`
				FROM `{$wpdb->prefix}skyav3_scan_files`
			...
			if (intval($wpdb->get_var("SELECT COUNT(*) FROM `{$wpdb->prefix}skyav3_scans` WHERE `status`='finished' AND NOT(LENGTH(`error`))")) > 1) {
				$statuses[] = 'new';
			}
			...
			$dbResult = $wpdb->get_results("
				SELECT
					...
					f.size
				FROM
					`{$wpdb->prefix}skyav3_scan_warnings` AS w LEFT JOIN
					`{$wpdb->prefix}skyav3_scan_files` AS f ON
	...
	public function get_status($formatted=false) {
		if (!$this->is_scan()) {
			return $formatted ? __('Invalid scan.', SKYELLSAV_L10N) : '';
		}
		...
		$translated = array(
			'created'=>__('Initializing the scan.', SKYELLSAV_L10N),
			'building-core'=>__('Collecting WordPress checksums.', SKYELLSAV_L10N),
			'building-plugins'=>__('Collecting plugin checksums.', SKYELLSAV_L10N),
			'building-themes'=>__('Collecting theme checksums.', SKYELLSAV_L10N),
			'building-local'=>__('Searching the local filesystem.', SKYELLSAV_L10N),
			'building-history'=>__('Finishing up the pre-scan.', SKYELLSAV_L10N),
			'scanning'=>__('Scanning files.', SKYELLSAV_L10N),
			'finished'=>__('The scan has completed.', SKYELLSAV_L10N),
			'finished-other'=>__('The scan encountered an error.', SKYELLSAV_L10N),
			'finished-timeout'=>__('The scan timed out.', SKYELLSAV_L10N),
			'finished-user-abort'=>__('The scan has been aborted.', SKYELLSAV_L10N),
		);
	...
	public function finish($error=null) {
		...
		delete_transient('skyav_scan_abort');
		delete_transient('skyav_scan_lock');
	...
	public function scan() {
		...
		if (false !== get_transient('skyav_scan_abort')) {
	...
	protected function scan_build_local() {
		...
		foreach ($inserts as $v) {
			$wpdb->query("
				INSERT INTO `{$wpdb->prefix}skyav3_scan_files` (`scan_id`, `file_hash`, `file_path`)
				VALUES " . implode(',', $v) . '
	...
	protected function scan_build_history() {
		...
		$wpdb->query("
			INSERT INTO `{$wpdb->prefix}skyav3_scan_files` (`scan_id`, `file_hash`, `file_path`, `type`)
			(
				SELECT
					$scan_id AS `scan_id`,
					`file_hash`,
		...
		if ($previous) {
			$wpdb->query("
				INSERT INTO `{$wpdb->prefix}skyav3_scan_files` (`scan_id`, `file_hash`, `file_path`, `checksum_expected`)
				(
					SELECT
						$scan_id AS `scan_id`,
						`file_hash`,
		...
		$wpdb->query("
			UPDATE
				`{$wpdb->prefix}skyav3_scan_files` AS f,
				`{$wpdb->prefix}skyav3_core` AS c
		...
		if (count($conds)) {
			...
			$wpdb->query("
				UPDATE `{$wpdb->prefix}skyav3_scan_files`
				SET `status`='skipped'
				WHERE
					`scan_id`=$scan_id AND
					($conds)
	...
	protected function scan_warnings($file_hash) {
		...
		if (count($inserts)) {
			$wpdb->query("
				INSERT INTO `{$wpdb->prefix}skyav3_scan_warnings` (`scan_id`,`file_hash`,`warning`)
	...
	public function email() {
		...
		$body = sprintf(
			__('The %s file scan for %s completed in %s.', SKYELLSAV_L10N),
			$mode,
			$site_name,
		...
		if ($results['error']) {
			$body .= ' ' . __('Unfortunately this scan was not successful due to an error.', SKYELLSAV_L10N);
		}
		else {
			$body .= "\n";
			$body .= "\n\t" . __('Files', SKYELLSAV_L10N) . ": {$results['results']['total']}";
			$body .= "\n\t" . __('Scanned', SKYELLSAV_L10N) . ": {$results['results']['scanned']}";
			$body .= "\n\t" . __('Skipped', SKYELLSAV_L10N) . ": {$results['results']['skipped']}";
			$body .= "\n\t" . __('Warnings', SKYELLSAV_L10N) . ": {$results['results']['warnings']}";
			...
			if ($results['results']['warnings']) {
				...
				foreach ($results['details']['warnings'] as $v) {
					$data[] = array(
						__('Warning', SKYELLSAV_L10N)=>$v['warningText'],
						__('Core', SKYELLSAV_L10N)=>($v['type'] ? __('Yes', SKYELLSAV_L10N) : __('No', SKYELLSAV_L10N)),
						__('File', SKYELLSAV_L10N)=>$v['file'],
					);
				}

				$filename = "sky-av_{$this->get_scan_id()}_" . __('warnings', SKYELLSAV_L10N) . '.csv';
			...
			if (
				...
				$fields = array(
					'modified'=>__('Modified', SKYELLSAV_L10N),
					'deleted'=>__('Deleted', SKYELLSAV_L10N),
					'new'=>__('New', SKYELLSAV_L10N),
				);
				$data = array();
				foreach ($fields as $k=>$v) {
					foreach ($results['details'][$k] as $v2) {
						$data[] = array(
							__('Status', SKYELLSAV_L10N)=>$v,
							__('Core', SKYELLSAV_L10N)=>($v2['type'] ? __('Yes', SKYELLSAV_L10N) : __('No', SKYELLSAV_L10N)),
							__('File', SKYELLSAV_L10N)=>$v2['file'],
						);
					}
				}

				$filename = "sky-av_{$this->get_scan_id()}_" . __('file-changes', SKYELLSAV_L10N) . '.csv';
				$filename = wp_unique_filename(files::get_tmp_dir(), $filename);
				$file = files::get_tmp_dir() . $filename;
				$data = common\format::to_csv($data);
				@file_put_contents($file, $data);
		...
		$body .= "\n\n" . admin_url('admin.php?page=skyav-scan');
		$body .= "\nSkytells-AV Security Scanner";

		wp_mail(
			$site_email,
			"[$site_name] Skytells-AV " . common\mb::ucwords($mode) . ' ' . __('Scan', SKYELLSAV_L10N),
			$body,
			null,
			$attachments
		);
	...
	public static function prune($max=null) {
		...
		// Delete some data!
		$wpdb->query("DELETE FROM `{$wpdb->prefix}skyav3_scans` WHERE `scan_id` IN ($old)");
		$wpdb->query("DELETE FROM `{$wpdb->prefix}skyav3_scan_files` WHERE `scan_id` IN ($old)");
		$wpdb->query("DELETE FROM `{$wpdb->prefix}skyav3_scan_warnings` WHERE `scan_id` IN ($old)");
	...
	public static function is_locked($type=null) {
		...
		// No lock exists.
		if (false === ($who = get_transient('skyav_scan_lock'))) {
			return false;
		}
	...
	public static function set_lock($type=null) {
		...
		if ($me) {
			set_transient('skyav_scan_lock', $me, 600);
		}
	}
