<?php
/**
 * Skytells-AV Security Scanner AJAX handlers.
 *
...
class ajax {
	...
	public static function init() {
		...
		foreach (static::ACTIONS as $action) {
			add_action("wp_ajax_skyav_ajax_$action", array($class, $action));
		}
	...
	protected static function parse(&$data, $nonce=true, $pro=false) {
		...
		if ($nonce) {
			if (!isset($data['n']) || !wp_verify_nonce($data['n'], static::NONCE)) {
				$out['errors']['other'] = __('The form had expired. Please try again.', SKYELLSAV_L10N);
				$out['status'] = 400;
			}
		}
		...
		if (!is_user_logged_in()) {
			$out['errors']['other'] = __('You must be logged in to complete this action.', SKYELLSAV_L10N);
			$out['status'] = 401;
		}
		elseif (!current_user_can('manage_options')) {
			$out['errors']['other'] = __('You are not authorized to complete this action.', SKYELLSAV_L10N);
			$out['status'] = 403;
		}

		// Require pro?
		if ($pro && !options::is_pro()) {
			$out['errors']['pro'] = __('A premium license is required.', SKYELLSAV_L10N);
			$out['status'] = 403;
		}
	...
	public static function file_delete() {
		...
		if (!count($out['errors'])) {
			...
			if (false === ($file = static::file_by_action($data['scan_id'], $data['file_hash'], 'delete'))) {
				$out['errors']['scan'] = __('The file could not be deleted. Please use FTP or SSH instead.', SKYELLSAV_L10N);
			}
			else {
	...
	public static function file_fix() {
		...
		if (!count($out['errors'])) {
			...
			if (false === ($file = static::file_by_action($data['scan_id'], $data['file_hash'], 'fix'))) {
				$out['errors']['scan'] = __('The file could not be automatically fixed.', SKYELLSAV_L10N);
			}
			else {
				$chmod = files::chmod();
				...
				if (
					...
					$out['data']['success'] = true;
					$wpdb->query("
						DELETE FROM `{$wpdb->prefix}skyav3_scan_warnings`
						WHERE
	...
	public static function file_ignore() {
		...
		if (!count($out['errors'])) {
			...
			if (false === ($file = static::file_by_action($data['scan_id'], $data['file_hash'], 'ignore'))) {
				$out['errors']['scan'] = __('An ignore rule could not be created automatically; please try adding it yourself.', SKYELLSAV_L10N);
			}
			else {
	...
	public static function file_view() {
		...
		if (!count($out['errors'])) {
			...
			if (false === ($file = static::file_by_action($data['scan_id'], $data['file_hash'], 'view'))) {
				$out['errors']['scan'] = __('The requested file cannot be viewed. Please use FTP instead.', SKYELLSAV_L10N);
			}
			else {
		...
		if (!count($out['errors'])) {
			...
			// Maybe a real one.
				...
				if ($license->is_license()) {
					...
					$out['errors']['license'] = __('The license is not valid.', SKYELLSAV_L10N);
				}
			}
		}
	...
	public static function scan() {
		...
		if (!count($out['errors']) && scan::is_locked()) {
			$out['errors']['other'] = __('Someone else is currently running their own scan. Please try again in a couple minutes.', SKYELLSAV_L10N);
		}
		...
		if (!count($out['errors'])) {
			...
			if (!is_object($scan) || !$scan->is_scan()) {
				$out['errors']['other'] = __('Invalid scan.', SKYELLSAV_L10N);
				$out['status'] = 404;
			}
			else {
				$scan::set_lock();
	...
	public static function scan_abort() {
		...
		if (!count($out['errors'])) {
			if (false !== ($scan = scan::get_open_scan())) {
				$out['data']['success'] = true;
				set_transient('skyav_scan_abort', 1, 600);
			}
		}
